...
func UpdateFromIndicePA(elasticClient *es.Client) error {
	...
	lines, err := readCSVFromURL("https://www.indicepa.gov.it/public-services/opendata-read-service.php?dstype=FS&filename=pec.txt")
	if err != nil {
		return err
	}
	...
	err = elastic.CreateIndexMapping(viper.GetString("indicepa_pec"), elastic.IPAMapping, elasticClient)
	if err != nil {
		return err
	}
	...
	for n, amm := range records {
		req := es.NewBulkIndexRequest().
			Index(viper.GetString("indicepa_pec")).
			Id(strconv.Itoa(n)).
			Doc(amm)
		bulkRequest.Add(req)
	}
